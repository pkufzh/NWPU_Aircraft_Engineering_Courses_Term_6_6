...
for i=1:n_spar
    for j=1:n_rib
        Vol_Spar_pla=Vol_Spar_pla+(dis_long(j)*t_pla_alum(i,j)*L_spar_h_abs(i,j)); % 梁腹板体积累加
        Vol_Spar_str=Vol_Spar_str+(dis_long(j)*(A_str_lower(i,j)+A_str_upper_002(i,j))); % 梁缘条体积累加
    end
end
Vol_Spar=Vol_Spar_pla+Vol_Spar_str;
...
% 计算元件总体积为Vol_Goal，总质量为M_Goal
Vol_Goal=(Vol_Skin+Vol_Rib+Vol_Spar)*rou_alum;
M_Goal=Vol_Goal*rou_alum;

%% 模块4：尺寸计算结果输出
fprintf('【UAV机翼主要元件尺寸计算（2020/07/12）】\n');
fprintf('\n');
fprintf('【蒙皮尺寸】\n');
fprintf('\n');
ts=max(t_skin);
