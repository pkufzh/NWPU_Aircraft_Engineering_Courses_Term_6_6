...
%     Area_room(n_room,k)=0.5*((1-L_spar(n_spar))*c_rib_all(k))*L_spar_h_abs(n_spar,k); % 后缘闭室近似为三角形
    for i=1:(n_spar-1)
        ...
        Room_area(i+1,k)=(0.5*ht)*dl;
        Room_C(i+1,k)=dl+ht+sqrt(dl^2+dh^2);
    end
end
...
% LY12铝合金材料属性
E_alum=70e9; % 拉伸杨氏模量（单位：Pa）
sigma_b_alum=420e6; % 拉伸强度极限（单位：Pa）
sigma_e_alum=280e6; % 受压失稳临界应力（单位：Pa）
sigma_s_alum=274e6; % 屈服极限（单位：Pa）
...
for i=1:n_rib
    ...
    n=(0.31*2*Mh*g*sqrt(bth^2-x.^2)/(pi*(bth^2)))-(0.24*2*Mw*g*sqrt((bth^2)-x.^2)/(pi*(bth^2)));
    q=int(z,x,bth,L);
    m=int(q,x,bth,L);
    mt=int(n,x,bth,L);
    p_q=double(q);
...
% 计算由扭矩引起的各翼肋处各翼梁的剪流q0
for k=1:n_rib
    q0_pla(n_spar,k)=(-1)*qt_room(n_spar,k);
    for i=1:(n_spar-1)
        q0_pla(i,k)=qt_room(i+1,k)-qt_room(i,k);
    end
end
...
A_str_upper=zeros(n_spar,n_rib); % 保存各翼肋站位处下梁缘条截面积（受拉伸）
la_str_lower=zeros(n_spar,n_rib);
lb_str_lower=zeros(n_spar,n_rib);
la_str_upper=zeros(n_spar,n_rib);
lb_str_upper=zeros(n_spar,n_rib);
...
% 确定上缘条尺寸——受压（由压杆总体稳定性确定）
for k=1:n_rib
    for i=1:n_spar
        A_str_upper(i,k)=(N_str(i,k)/sigma_e_alum)/Ke_str;
        la_str_upper(i,k)=0.08;
        lb_str_upper(i,k)=A_str_upper(i,k)/la_str_upper(i,k);
    end
end

%% 模块4：尺寸计算结果输出
fprintf('【UAV机翼主要元件尺寸计算（2020/06/24）】\n');
fprintf('\n');
fprintf('【蒙皮尺寸】\n');
fprintf('\n');
ts=max(t_skin);
...
for i=1:n_spar
    ta=max(A_str_upper');
    fprintf('梁%d# 上缘条截面积为：%.4f（mm^2）\n',i,ta(i)*e*e);
end
for i=1:n_spar
    la=max(la_str_upper');
    fprintf('梁%d# 上缘条宽度为：%.4f（mm）\n',i,la(i)*e);
end
for i=1:n_spar
    lb=max(lb_str_upper');
    fprintf('梁%d# 上缘条高度为：%.4f（mm）\n',i,lb(i)*e);
end
fprintf('\n');
